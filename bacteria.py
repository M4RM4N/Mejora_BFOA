    ...
    def mutacion_adaptativa(self, poblacion, worst_idx_list, prob_mut=0.7):
        for idx in worst_idx_list:
            if random.random() < prob_mut:
                individuo = list(poblacion[idx])  # convierte a lista de listas
                nuevo = []
                for secuencia in individuo:
                    nueva_sec = secuencia[:]
                    if len(nueva_sec) == 0:
                        continue
                    pos = random.randint(0, len(nueva_sec) - 1)
                    if nueva_sec[pos] != "-":
                        nueva_sec.insert(pos, "-")  # inserta gap
                    elif len(nueva_sec) > 1:
                        nueva_sec.pop(pos)  # elimina gap
                    nuevo.append(nueva_sec)
                poblacion[idx] = tuple(nuevo)
